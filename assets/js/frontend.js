...
var callback = function (response) {
  if (null === response.error) {
    ...
    alert(
      "An error occured: " + response.message + "(Code: " + response.error + ")"
    );
    return false;
  }
};
jQuery(function () {
  ...
  jQuery(fc).on("click", pl, function (e) {
    ...
    if (
      ...
      var year =
        currentYear.substring(0, 2) + "" + jQuery(".expiry-year").val();
      jQuery(".expiry-year").val(year);
      console.log(
        apiUrl +
          syspay_params.publicKey +
          jQuery(".card-number").val() +
          jQuery(".name").val() +
          jQuery(".expiry-month").val() +
          jQuery(".expiry-year").val() +
          jQuery(".cvc").val()
      );
